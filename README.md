...
| **AWS Lambda** | - Hosts the **AI inference logic** using a lightweight **ONNX model** to predict `set_temp` and `fan_speed` values based on incoming telemetry (`temperature`, `humidity`, `occupancy`).<br>- Publishes predicted commands back to IoT Core topics for simulators to consume.<br>- Stores raw and processed telemetry into **DynamoDB** for quick lookups.<br>- **Note:** A more scalable approach could use **Amazon SageMaker** or an **ECS container** running the AI logic, but to keep this example simple, **AWS Lambda** is used here. |
